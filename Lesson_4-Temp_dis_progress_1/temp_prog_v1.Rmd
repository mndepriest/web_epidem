...
# Get the width (time)
w1 <- (sub.dta1$dai[1] + (sub.dta1$dai[1+1]))
w1
...
# This can be done in fewer steps:
a1 <- (sub.dta1$dai[1] + (sub.dta1$dai[1+1]))*((sub.dta1$prop[1]+sub.dta1$prop[1+1])/2)
a1
...
for (i in as.numeric(1:3)){
  ...
  result <- (sub.dta1$dai[i] + (sub.dta1$dai[i+1]))*((sub.dta1$prop[i]+sub.dta1$prop[i+1])/2)
...
AUDPC <- function(times, proportions) {
  ...
  # Calculate the area for each rectangle
    ...
    result <- (times[i] + times[i+1])*((proportions[i]+proportions[i+1])/2)
