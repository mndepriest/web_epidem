...
author: "Paul Esker; Mauricio Serrano; Felipe Dalla Lana, Miranda DePriest"
output:
  html_document:
    code_folding: show
    toc: true
...
First, we will load in `ggplot2` and our example data from the last lesson (Intro to R). Remember that `ggplot2` is a core package in `tidyverse`, so it can be loaded with `library("ggplot2")` or `library("tidyverse")`. We'll also include `conflicted()` since there are some instances of different packages using identical functions. 

```{r}
# Load tidyverse (which includes ggplot2) and conflicted packages
library(tidyverse)
library(conflicted)

# Tell R which version of `filter()` you want to use
conflicts_prefer(dplyr::filter)

# Load in the example data and inspect it
...
**Note:** You can inspect the data more closely by clicking on it in the Global Environment. This panel is typically in the upper right corner. 
...
`ggplot2` contains 10 pre-set themes and has options to add further customization. Other packages like [`ggthemes()`](https://yutannihilation.github.io/allYourFigureAreBelongToUs/ggthemes/) offer more theme pre-sets. Themes can be useful when showing plots in different settings, such as with projectors, on colored paper, or in journals with figure specifications. 
