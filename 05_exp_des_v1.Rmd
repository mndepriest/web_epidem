...
When designing an experiment, the key objectives are minimizing bias and ensuring the reproducibility of the work. It is important to think about the analyses you will want to do with the results before you even think about setting the experiment up. Otherwise, the results may not address the questions they were meant to. 
...
Although our assumption of spatial homogeneity is rarely, if ever, fulfilled, this experimental design still strongly controls for heterogeneity. The likelihood of most or all subjects receiving a certain treatment being located within the same group of locations (and the same environmental heterogeneity) is relatively low. This allows us to untangle the impact of the treatment versus the impacts of environmental heterogeneity. An example of using a completely randomized design is shown next on some example data. 

First, we load in the necessary packages for the example and the rest of the lesson. 
...
To prevent bias from environmental heterogeneity like the tree, we can use a CRD in our experiment. By randomizing the location of each of our experimental units, we can decrease the likelihood of mistaking effects of a more shady environment with the effects of the treatment. Because treatments are randomly assigned to plots, it's very unlikely that most or all replicates of a single treatment are subjected to a different environment. This is illustrated below. 

```{r}
# Insert CRD_no_random and CRD_trt
```
...
First, we will transform the data and then create a plot showing the location of our experimental units in the plot arrangement. 
...
Now we have a map detailing where each experimental unit goes in a completely randomized design. We can use tools like `agricolae` to create planting maps to follow in our experiments. Now, if there's environmental heterogeneity like a tree shading the first column of the plot arrangement, it won't just impact one or two treatments. We have prevented ourselves from attributing the impact of something in the environment to our treatment and producing an erroneous result.

<br>

### **Randomized Complete Block Design (RCBD)**

In a Randomized Complete Block Design (RCBD), we further protect our results from bias by constraining randomization within blocks, or 'blocking'. When we do this, we assume that conditions within a block (an experimental location, such as a field) are homogeneous, but that conditions *between* different blocks are not. Because of this, we *randomly assign treatments* **within* *blocks*. This ensures that no single treatment is impacted by heterogeneous conditions at a location, and that there is a representative from each treatment at each location, experiencing the same environmental conditions. This is a basic constrained design from which more complicated experimental designs are often constructed. 

First, we'll create example data. In this data, we have *6 treatments* represented by letters A-F, *4 replicates* of each treatment, and *4 unique blocks/locations*. You can imagine that the 4 blocks are plots in different fields that experience different weather. Within each block, we have a number representing each of the *6 plots within each block* (for a total of 24 individual plots). We will also set the seed number so that your example data is identical to ours. 
...
Now, we transform the data to show row and column locations. We also created 'id' which identifies individual plots. Each plot is indicated by its block number and plot number (e.g., plot B4-#2 would be plot 2 in block 4). Finally, we'll see how this experimental design is arranged with a plot. 
...
Essentially, we have drastically reduced the bias in our experiment, as is shown in the image below.
...
This is one example of a RCBD experimental design. Depending on the circumstances of your experiment, having blocks made of a single row may not be the most appropriate. In some instances, changing the block shape may better represent what areas are likely to be more similar, allowing you to create a more meaningful block arrangement. 

In the previous example, we assumed that our plots occurred in a row in different fields, regardless of the fields' proximity to each other. Now, consider a slightly different circumstance: Imagine you have four fields (blocks) where there are both rows and columns of available plots, like in the next image. We can easily modify our previous RCBD design to accommodate these conditions. 
...
```{r}
# Factorial CRD concept image showing sprayer limitations
```
...
```{r}
# Concept figure with 2 panes, 1 showing wind blowing from trt A onto trt B and the 2nd showing the plants all randomized and blowing on each other. 
```
...
```{r}
#Concept image showing normal distribution, skewed distribution, etc.
```
...
# Regression
RCBD_yld <- lm(yld ~ trt + blk, data = yld_data)
summary(RCBD_yld)
...
The reports generated by these tests are quite similar in format. For more information on interpreting these reports, use the `?()` or `help()` tools to see their documentation. The important part of the results is in the `$groups` section. Treatment name, yield, and a group identifier are presented. The group identified, a letter, indicates groups where the means are not significantly different. In this case, treatments B, C, and D are in group a, meaning that they are not significantly different. Treatment A is in a different group, showing that its yield data *is* significantly different. 
